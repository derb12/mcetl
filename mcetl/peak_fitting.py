...
def generate_lmfit_model(params_dict):
    ...
    Parameters
    ----------
    params_dict : a dictionary with peak prefixes as keys and a list of the
                  peak model string and the peak parameters as the values.

    Returns
    -------
    composite_model : an lmfit CompositeModel containing all of the peaks
    params : an lmfit Parameters object containing the parameters for all peaks
    """
...
def find_peak_centers(x, y, additional_peaks=None, height=None,
    ...
    Parameters
    ----------
    x : x-values for the raw data
    y : y-values for the raw data
    additional_peaks : peak centers that are input by the user and
                       automatically accepted as peaks if within x_min and x_max
    height : height that a peak must have for it to be considered a peak by
             scipy's find_peaks
    prominence : prominence that a peak must have for it to be considered
                 a peak by scipy's find_peaks
    x_min : minimum x value used in fitting the data
    x_max : maximum x value used in fitting the data

    Returns
    -------
    peaks_found : a list of all the peak centers found
    peaks_accepted : a list of peak centers that were accepted because they
                     were within x_min and x_max
...
def find_hidden_peaks(x, fit_result, peak_centers, peak_fwhms,
    ...
    Parameters
    ----------
    x : x-values for the fitting
    fit_result : an lmfit ModelResult object from the last fitting
    peak_centers : a list of peak centers that are already within the model
    peak_fwhms : a list of fwhm or estimated fwhm for each peak in the model
    min_resid : the minimum prominence that a peak must have for it to be considered
                a peak by scipy's find_peaks; the actual prominence used by find_peaks
                is min_resid * (max_y - min_y)
    debug : if True, will plot the residuals, and the residuals after smoothing

    Returns
    -------
    residual_peaks_found : a list of the centers of all residual peaks found
                           from peak fitting the residuals
    residual_peaks_accepted : a list of the centers for the residual peaks
                              accepted into the model; in order to be accepted,
                              the residual peak must fulfill the following condition:
                                  x_l + fwhm_l/2 < x_peak < x_r - fwhm_r/2
                              where x_l and x_r are the centers of the peaks to
                              the left and right of the found residual peak (with
                              center at x_peak), and fwhm_l and fwhm_r are the full
                              width at half maximum of the left and right peaks. It
                              is an arbitrary condition that has no real basis in science,
                              but is just meant to reduce the number of residual peaks
                              accepted
...
def re_sort_prefixes(params_dict):
    ...
    Returns
    -------
    new_params_dict: a dictionary with peak prefixes as keys and a list of the
                     peak model string and the peak parameters as the values,
                     but sorted so that the peak prefixes are in order from
                     left to right
...
def plugNchug_fit(x, y, height=None, prominence=np.inf, center_offset=1.0,
    ...
    prominence : prominence that a peak must have for it to be considered
                 a peak by scipy's find_peaks
    center_offset : value that determines the min and max parameters for
                    the center of each peak. min = center - center_offset,
                    max = center + center_offset
    peak_width : Used in initializing each peak. When first estimating the
                 peak parameters, only the data from x - peak_width/2 to
                 x + peak_width/2 is used to fit the peak.
    default_model : model used if the model_list is None or does not have
                    enough values for the number of peaks found
    subtract_background : if True, it will fit the background with the model
                          in background_type
    bkg_min : minimum x value to use for initially fitting the background
    bkg_max : maximum x value to use for initially fitting the background
    min_sigma : minimum value for the sigma for each peak; typically
                better to not set to any value other than 0
    max_sigma : maximum value for the sigma for each peak; typically
                better to not set to any value other than infinity
    min_method : minimization method used for fitting
    x_min : minimum x value used in fitting the data
    x_max : maximum x value used in fitting the data
    additional_peaks : peak centers that are input by the user and
                       automatically accepted as peaks
    model_list : list of strings, with each string corresponding to one of the
                 models in lmfit.models
    background_type : string corresponding to a model in lmfit.models; used
                      to fit the background
    poly_n : degree of the polynomial; only used if background_type is
             set to 'PolynomialModel'
    fit_kws : keywords to be passed on to the minimizer
    vary_Voigt : if True, will allow the gamma parameter in the Voigt model
                 to be varied as an additional variable; if False, gamma
                 will be set equal to sigma
    fit_residuals : if True, it will attempt to fit the residuals after the
                    first fitting to find hidden peaks, add them to the model,
                    and fit the data again
    num_resid_fits : maximum number of times the program will loop to fit
                     the residuals
    min_resid : used as the prominence when finding peaks in the
                residuals, in which the prominence is set to
                min_resid * (y_max - y_min)
    debug : if True, will create plots at various portions of the code showing
            the peak initialization, initial fits and backgrounds, and residuals
    peak_heights : a list of peak heights

    Returns
    -------
    output : a dictionary of lists. For most lists, each entry corresponds to
    ...
    it has quite a dense input and output, but it is well worth it!
        ...
        Should eventually just make this take *args and **kwargs so that the input
    is not so large.
...
def r_squared(y, y_calc, num_variables):
    ...
    Parameters
    ----------
    y : the experimental y data
    y_calc : the calculated y from fitting
    num_variables : the number of variables used by the fitting model

    Returns
    -------
    r_sq : the r squared value for the fitting
    r_sq_adj : the adjusted r squared value for the fitting, which takes into
...
def background_selector(x_input, y_input, click_list=None):
    """Allows selection of background on a matplotlib plot.

    Parameters
    ----------
    x_input : the input data for x
    y_input : the iput data for y
    click_list : A nested list, with each entry corresponding to [x, y] locations
    ...
    def create_circle(x, y, axis):
        """Places a cirle designating the peak center and height on the axis

        Parameters
        ----------
        x : the x location on the axis to place the circle
        y : the y location on the axis to place the circle
        axis : the axis in which to put the circle
        """
    ...
    def plot_background(x, y, axis, axis_2):
        ...
        Parameters
        ----------
        x : the x values of the raw data
        axis : the axis upon which to plot the background
        """
    ...
    def on_click(event):
        ...
        Parameters
        ----------
        event : the button_press_event event
    ...
    def on_pick(event):
        """The function to be executed if an object is picked

        Parameters
        ----------
        event : the pick_event event
...
def peak_selector(x_input, y_input, click_list=None, initial_peak_width=1,
    ...
    x_input : the input data for x
    y_input : the iput data for y
    click_list : A nested list, with each entry corresponding to a peak. Each entry
                 has the following layout:
                     [[lmfit model, sigma fct, aplitude fct],
                      [peak center, peak height, peak width]]
    ...
    initial_peak_width : the initial peak width input in the plot
    subtract_background : if True, will subtract the background before showing the plot
    background_type : string corresponding to a model in lmfit.models; used
                      to fit the background
    poly_n : degree of the polynomial; only used if background_type is
             set to 'PolynomialModel'
    bkg_min : minimum x value to use for initially fitting the background
    bkg_max : maximum x value to use for initially fitting the background
    default_model : the initial model to have selected on the plot
    ...
    def plot_total_peaks(x, axis):
        ...
        ----------
        x : the x values of the raw data
        axis : the axis upon which to plot the peaks
        """
...
def plot_confidence_intervals(x, y, fit_result, n_sig=3):
    ...
    Parameters
    ----------
    x : x data used in the fitting
    y : y data used in the fitting
    fit_result : a ModelResult object from lmfit
    n_sig : an integer describing the multiple of the standard error to use
...
def plot_peaks_for_model(x, y, x_min, x_max, peaks_found, peaks_accepted,
    ...
    x : x data used in the fitting
    y : y data used in the fitting
    x_min : minimum x values used for the fitting procedure
    x_max : maximum x values used for the fitting procedure
    peaks_found : a list of x values corresponding to all peaks found throughout
                  the peak fitting and peak finding process, as well as those
                  input by the user
    peaks_accepted : a list of x values corresponding to peaks found throughout
                     the peak fitting and peak finding process that were accepted
                     into the model
    additional_peaks : a list of peak centers that were input by the user
    """
...
def plot_fit_results(x, y, fit_result, label_rsq=False, plot_initial=False):
    ...
    x : x data used in the fitting
    y : y data used in the fitting
    fit_result : a ModelResult object from lmfit; can be a list of ModelResults,
                 in which case, the initial fit will use fit_result[0], and the
                 best fit will use fit_result[-1]
    label_rsq : if True, will put a label with the adjusted r squared value
                of the fitting
    plot_initial : if True, will plot the initial fitting as well as the best fit
    """
...
def plot_individual_peaks(x, y, individual_peaks, fit_result, background_subtracted=False,
    ...
    x : x data used in the fitting
    y : y data used in the fitting
    individual_peaks : a dictionary with keys corresponding to the model prefixes
                       in the fitting, and their values corresponding to their
                       y values
    fit_result : a ModelResult object from lmfit
    background_subtracted : whether or not the background was subtracted in
                            the fitting
    plot_subtract_background : if True, subtracts the background from the raw data
                               as well as all the peaks, so everything is nearly flat
    plot_separate_background : if True, subtracts the background from just the individual
                               peaks, while also showing the raw data with the
                               composite model and background
    plot_w_background : if True, has the background added to all peaks, i.e. it shows
...
if __name__ == '__main__':
    ...
    fitting_results = plugNchug_fit(x_array, y_array,
                                    height=rel_height, prominence=prominence,
                                    center_offset=center_offset,
                                    peak_width=peak_width, model_list=model_list,
                                    subtract_background=subtract_background,
                                    x_min=x_min, x_max=x_max,
                                    additional_peaks=additional_peaks,
                                    background_type=background_type,
                                    poly_n=poly_n, min_method=min_method,
                                    default_model=default_model,
                                    fit_kws=fit_kws, vary_Voigt=vary_Voigt,
                                    fit_residuals=fit_residuals,
                                    num_resid_fits=num_resid_fits,
                                    min_resid=min_resid,
